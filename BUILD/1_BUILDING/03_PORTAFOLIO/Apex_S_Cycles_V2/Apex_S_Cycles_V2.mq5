...
void VisualDebug() {
   ...
   ObjectSetInteger(0, "DB_F_LOW(0)", OBJPROP_COLOR, clrTomato);
...
void ManageSniper() {
   ...
   if(sl == 0) {
      double initial_sl = (active_dir == 1) ? entry - dynamic_sl_dist : entry + dynamic_sl_dist;
...
void ManageMaximizer() {
   ...
   double min_dist = MathMax(stop_level, freeze_level) + (_Point * 10);
